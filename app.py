...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///' + os.path.join(basedir, 'ticketing.db')
...
class User(UserMixin, db.Model):
    ...
    role = db.Column(db.String(50), nullable=False, default='user')  # user, agent, supervisor
...
def calculate_priority(impact, urgency):
    ...
    if impact == 'high' and urgency in ['critical', 'high']:
        return 'P1'
    if (impact == 'high' and urgency == 'medium') or (impact == 'medium' and urgency == 'critical'):
        return 'P2'
    if (impact == 'medium' and urgency in ['high', 'medium']) or (impact == 'low' and urgency == 'critical'):
        return 'P3'
    return 'P4'
...
def route_ticket(category, priority):
    ...
    if category.startswith('IT'):
        ...
        if priority in ['P1', 'P2']:
            user = User.query.filter_by(role='agent', department='IT L2').first()
            if user:
                return user.id
        # fallback L1
...
def check_escalation():
    now = datetime.utcnow()
    tickets = Ticket.query.filter(Ticket.status.notin_(['Closed', 'Cancelled'])).all()
    for t in tickets:
        if t.sla_response_target and now > t.sla_response_target:
            if t.escalation_level < 3:
                ...
                send_email(os.getenv('EMAIL_SENDER'), f'Ticket {t.id} Escalated', f'Ticket {t.id} breached response SLA.')
    db.session.commit()
...
def register():
    if request.method == 'POST':
        ...
        user = User(name=name, email=email, password=generate_password_hash(pw), role=role, department=dept)
        db.session.add(user)
        db.session.commit()
...
@login_required
def create_ticket():
    if current_user.role != 'user' and current_user.role != 'supervisor' and current_user.role != 'agent':
        flash('Access denied')
        return redirect(url_for('dashboard'))
    if request.method == 'POST':
        ...
        db.session.commit()
        send_email(current_user.email, f'Ticket {ticket_id} Created', 'Your ticket was submitted.')
        flash(f'Ticket {ticket_id} created')
        return redirect(url_for('dashboard'))
...
def resolve_ticket(ticket_id):
    ticket = Ticket.query.get_or_404(ticket_id)
    if current_user.role not in ['agent','supervisor'] and current_user.id != ticket.user_id:
        flash('Access denied')
        return redirect(url_for('dashboard'))
    ...
    if ticket.user_id:
        ...
        if user:
            send_email(user.email, f'Ticket {ticket_id} Resolved', 'Please confirm resolution.')
